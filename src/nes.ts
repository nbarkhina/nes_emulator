...
export class Nes {
    ...
    canvasDebug_ctx: CanvasRenderingContext2D;
    canvasDebug: HTMLCanvasElement;
    canvasDebug_Image: ImageData;
    ...
    //nestest rom
    nestestlog: string[] = [];
    nestestlogstatus: string[] = [];
    nestestcounter = 0;
    nestestlogmode: boolean = false; //ALSO GO TO CPU UNCOMMENT CYCLES
    nestestframecounter = 0;         //SHOULD REACH LINE 5003 TO PASS
                                     //ALSO GO TO frame() AND UNCOMMENT nestestlogmode CHECK
    //performance tuning
    timeDiff = 0;
    timeCalc = 0;
    timeDiffArray = []; //only used for average mode
    ...
    constructor() {
        ...
        this.fps_controller = new FPSController(60);
    ...
    initCanvases() {
        ...
        this.SCREEN_MAIN = new VideoScreen(this.SCREEN_WIDTH, this.SCREEN_HEIGHT, this.canvas_ctx, this);
    }
    ...
    enableDebugCanvas(){
        ...
        this.SCREEN_DEBUG = new VideoScreen(this.SCREEN_DEBUG_WIDTH, this.SCREEN_DEBUG_HEIGHT, this.canvasDebug_ctx, this);

    }


    loadROM(rom_data: string, rom_name: string) {
    ...
    */
    ...
    frame() {
        ...
        //for calculating via average
        //however other method shows range
        //of values better
        /*if (this.timeCalc % 60 == 0){
            //average and flush array
            this.timeDiffAvg = this.timeDiffArray.reduce((prev,current)=>{ return prev+current; })/this.timeDiffArray.length;
            this.timeDiffArray = [];
        }
        this.timeDiff = window.performance.now() - perfCheck;
        this.timeDiffArray.push(this.timeDiff);*/
    ...
    load_state_process(){
        ...
        if (this.loadStateData!=null){
            ...
            SaveState.parseLoad(this,this.loadStateData);
        ...
        for (let i = startingSprite; i < 64; i++) {
            ...
            if (x_position < 0 || x_position >= this.SCREEN_MAIN.SCREEN_WIDTH ||
                ...
                if (this.ppu.sprite_size == 0) {
                    ...
                    this.ppu.renderTile((tile_number * 16) + offset, x_position, y_position,
                        this.SCREEN_MAIN, 0, 0, null, null, attributes);
                }
                //8x16 Tiles Mode
                else {
                    ...
                    let chr_table = BitHelper.getBit(tile_byte, 0);
                    if (chr_table == 1)
                        tile_number += 256;
                    ...
                    let flip_v = BitHelper.getBit(attributes,7);
                    if (flip_v)
                    {
                        tile1 = tile_number + 1;
                        tile2 = tile_number
                    ...
                    //top tile
                    this.ppu.renderTile(tile1 * 16, x_position, y_position,
                        this.SCREEN_MAIN, 0, 0, null, null, attributes);
                    //bottom tile
                    this.ppu.renderTile(tile2 * 16, x_position, y_position + 8,
                        this.SCREEN_MAIN, 0, 0, null, null, attributes);
                }
            }
    ...
    drawBackground(nameTable: number) {
        ...
        if (this.cartridge.mirroring == Mirroring.HORIZONTAL) //ice climber style up_down
        {
        ...
        if (this.cartridge.mirroring == Mirroring.VERTICAL) //mario style left_right
        {
            ...
            for (let x = 0; x < 32; x++) {
                ...
                if (!isOffScreen) {
                    ...
                    if (nameTable == 1) {
                        ...
                        if (this.cartridge.mirroring == Mirroring.VERTICAL || this.cartridge.mapperType==7)
                    ...
                    if (this.ppu.pattern_background == 1)
                        offset = 256 * 16;
                    this.ppu.renderTile((byte * 16) + offset, x * 8, y * 8,
                        this.SCREEN_MAIN, scrolladjust_x + wraparound_x, scrolladjust_y + wraparound_y, nameTableCounter, nameTable);
                }
    ...
    applyScreenBuffers() {
        ...
        if (this.DEBUGMODE) {
            //prevent console errors while loading
            if (!this.canvasDebug_Image)
                return;
            else
                this.debuggerLoading = false;
            ...
            for (let y = 0; y < 30; y++) {
                for (let x = 0; x < 32; x++) {
                    ...
                    if (this.ppu.pattern_background == 1)
                        offset = 256 * 16;
                    this.ppu.renderTile((byte * 16) + offset, x * 8, (y * 8),
                        this.SCREEN_DEBUG, 0, 0, nameTableCounter, this.debugNametable);
                    nameTableCounter++;
    ...
    drawDebugInfo() {
        ...
        // stats += 'Scroll X: ' + this.ppu.scroll_x + '<br>';
        // stats += 'Scroll Y: ' + this.ppu.scroll_y + '<br>';
        ...
        if (this.apu._soundDisabled == false) {
            ...
            if (this.apu.square1.tone.volume.value > -100) {
                ...
                if (equalizerCount > 20) equalizerCount = 20;
                for (let i = 0; i < equalizerCount; i++) square1Equalizer += '*';
                square1Amount = Math.floor(this.apu.square1.debugCurrentFrequency / 2);
                if (square1Amount > 400) square1Amount = 400;
            }
            ...
            if (this.apu.square2.tone.volume.value > -100) {
                ...
                if (equalizerCount > 20) equalizerCount = 20;
                for (let i = 0; i < equalizerCount; i++) square2Equalizer += '*';
                square2Amount = Math.floor(this.apu.square2.debugCurrentFrequency / 2);
                if (square2Amount > 400) square2Amount = 400;
            }
            ...
            if (this.apu.triangle.tone.volume.value > -100) {
                ...
                if (equalizerCount > 20) equalizerCount = 20;
                for (let i = 0; i < equalizerCount; i++) triangleEqualizer += '*';
                triangleAmount = Math.floor(this.apu.triangle.debugCurrentFrequency / 2);
                if (triangleAmount > 400) triangleAmount = 400;
            }
            ...
            let note1 = Note.getNote(square1Amount);
            let note2 = Note.getNote(square2Amount);
            let note3 = Note.getNote(triangleAmount);

            if (note1!='') this.debugSquare1Note = note1;
            if (note2!='') this.debugSquare2Note = note2;
            if (note3!='') this.debugTriangleNote = note3;
    ...
    recordMusicMode = false;
    recordMusicArray: number[] = [];
    playBackMusicMode = false;
    playBackMusicCounter = 0;
