...
define(["require", "exports", "./input_controller", "./nes", "./dbexport"], function (require, exports, input_controller_1, nes_1, dbexport_1) {
    "use strict";
    ...
    class MyApp {
        constructor() {
            ...
            this.chkSound = true;
            this.chkDisableGamepad = false;
            this.chkForceDesktop = false;
            this.chkForceMobile = false;
            this.romdevMode = false;
            this.canvasDebugSize = 400;
            this.remapMode = '';
            this.chkUseJoypad = false;
            this.remappingPlayer1 = false;
            try {
            ...
            if (this.romdevMode) {
                if (window.location.href.indexOf('localhost') > 0) {
                    setTimeout(() => {
                        document.getElementById('romselect').selectedIndex = document.getElementById('romselect').length - 3;
                        this.loadRom();
                    }, 500);
                    setTimeout(() => {
        ...
        uploadImport(event) {
            ...
            reader.onload = function (e) {
                console.log('finished loading');
                let importString = this.result;
                app.importDatabase(importString);
            };
            reader.readAsBinaryString(file);
        }
        importDatabase(importString) {
            var request = indexedDB.open('NeilNESDB');
            request.onsuccess = function (ev) {
                var db = ev.target.result;
                let dbexport = new dbexport_1.DBExport();
        ...
        showCanvasAndInput() {
            ...
            if (this.mobileMode) {
                ...
                this.inputController = new input_controller_1.InputController();
                    ...
                    romStore.openCursor().onsuccess = function (ev) {
                        ...
                        if (cursor) {
                            ...
                            if (app.dblist.length > 0) {
                                let romselect = document.getElementById('dbselect');
        ...
        remapPressed() {
            if (this.remapMode == 'Key') {
                ...
                //player 2
                if (this.currKey == 11)
                    this.remappings.Mapping_Up_2 = keyLast;
                if (this.currKey == 12)
                    this.remappings.Mapping_Down_2 = keyLast;
                if (this.currKey == 13)
                    this.remappings.Mapping_Left_2 = keyLast;
                if (this.currKey == 14)
                    this.remappings.Mapping_Right_2 = keyLast;
                if (this.currKey == 15)
                    this.remappings.Mapping_Action_A_2 = keyLast;
                if (this.currKey == 16)
                    this.remappings.Mapping_Action_B_2 = keyLast;
                if (this.currKey == 18)
                    this.remappings.Mapping_Action_Start_2 = keyLast;
                if (this.currKey == 17)
                    this.remappings.Mapping_Action_Select_2 = keyLast;
            }
